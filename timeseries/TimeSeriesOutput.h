...
class OStreamVisitor : public boost::static_visitor<>
{
 private:
  ...
  const Spine::ValueFormatter& itsValueFormatter;
  int itsPrecision;
  LonLatFormat itsLonLatFormat;

 public:
  OStreamVisitor(std::ostream& outs, const Spine::ValueFormatter& valueformatter, int precision)
      : itsOutstream(outs),
...
class StringVisitor : public boost::static_visitor<std::string>
{
 ...
 public:
  StringVisitor(const Spine::ValueFormatter& valueformatter, int precision)
      : itsValueFormatter(valueformatter),
        itsPrecision(precision),
        itsLonLatFormat(LonLatFormat::LONLAT)
  {
