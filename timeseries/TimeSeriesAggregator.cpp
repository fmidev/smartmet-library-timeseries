...
Value StatCalculator::getStatValue(const DataFunction& func, bool useWeights) const
{
  try
  {
    // if result set contains missing values, then 'nan'-flag in DataFunction determines if
...
TimedValue time_aggregate(const TimeSeries& ts, const DataFunction& func, const boost::local_time::local_date_time& timestep)
{
  try
  {
